...
static void Send_Pro_Data(unsigned char *buf)
{
	...
	USART_Send_Buf(USART1,buf,pHeader->length);
...
void Pro_Receive_Interface(void)
{
	...
	SDKHeader RecHeader ;
	memcpy(&RecHeader, Uart1.RxDataBuf,sizeof(SDKHeader));
	...
	memcpy(&Heard_CRC32,&Uart1.RxDataBuf[RecHeader.length - _SDK_CRC_DATA_SIZE] ,_SDK_CRC_DATA_SIZE);
	if(sdk_stream_crc32_calc((unsigned char*)Uart1.RxDataBuf, RecHeader.length - _SDK_CRC_DATA_SIZE)!=Heard_CRC32) return;//整体校验
	...
	if(RecHeader.is_ack)
	{
		...
		switch(Ack.session_id)
		{
			case 0x02:
				if(Ack.seq_num==0)
				{
					unsigned short temp = (unsigned short)Uart1.RxDataBuf[13]<<Uart1.RxDataBuf[12];
					if(temp==0)
						Activation_Ack = 1;
				}
				break;
